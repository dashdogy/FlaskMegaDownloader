...
apt_install_base() {
  ...
  apt-get update
  apt-get install -y ca-certificates curl git gnupg python3 python3-venv python3-pip acl unar p7zip-full
}
...
setup_runtime_dirs() {
  ...
  if id -u plex >/dev/null 2>&1; then
    setfacl -R -m u:plex:rwx -m d:u:plex:rwx /srv/mega-downloads /srv/media/incoming || \
      warn "Could not apply Plex ACLs to default media roots. Check filesystem ACL support."
  else
    warn "User 'plex' does not exist yet; runtime downloads will apply Plex ACLs when that user is available."
  fi
}
...
ensure_auth_config() {
  ...
  generated_password="$("${APP_VENV}/bin/python" - "${CONFIG_FILE}" <<'PY'
from pathlib import Path
import re
import secrets
import sys
...
hash_match = re.search(r"(?m)^ADMIN_PASSWORD_HASH\s*=\s*(['\"])(.*?)\1\s*$", content)
if hash_match and hash_match.group(2).strip():
    print("")
    raise SystemExit(0)
...
lines = []
if not re.search(r"(?m)^AUTH_ENABLED\s*=", content):
    lines.append("AUTH_ENABLED = True")
if not re.search(r"(?m)^ADMIN_USERNAME\s*=", content):
    lines.append("ADMIN_USERNAME = 'admin'")
if hash_match:
    content = re.sub(
        r"(?m)^ADMIN_PASSWORD_HASH\s*=.*$",
        f"ADMIN_PASSWORD_HASH = {password_hash!r}",
        content,
        count=1,
    )
else:
    lines.append(f"ADMIN_PASSWORD_HASH = {password_hash!r}")
if lines:
    content = content.rstrip() + "\n\n# Generated by the Flask Mega Downloader update helper.\n" + "\n".join(lines) + "\n"
...
PY
  ...
  if [[ -n "${generated_password}" ]]; then
    ...
    warn "Generated built-in admin credentials for this install."
    warn "Username: admin"
    warn "One-time password: ${GENERATED_ADMIN_PASSWORD}"
    warn "Store this password now; it will not be printed again after ADMIN_PASSWORD_HASH exists."
  fi
}
